...
#define json_array_foreach(__array, __index, __element) \
        for(__index = 0; __index < json_object_array_length(__array) && (__element = json_object_array_get_idx(__array, __index)); __index++)
...
#define CONFIG_HAS(__config, __key, __out) ((__out = json_object_object_get(__config, __key)) != NULL)
...
#define CONFIG_OVERRIDE_BOOL(__config, __key, __value, __field_name, __warning) do { \
    ...
    if(_tmp && __warning) { \
        if(!json_object_is_type(_tmp, json_type_boolean)) \
            ...
            MARGO_WARNING(0, "Overriding field \"%s\" (\"%s\") with value \"%s\"", \
                    __field_name, json_object_get_boolean(_tmp) ? "true" : "false", \
                    __value ? "true" : "false"); \
    } \
    json_object_object_add(__config, __key, json_object_new_boolean(__value)); \
} while(0)
...
#define CONFIG_OVERRIDE_INTEGER(__config, __key, __value, __field_name, __warning) do { \
    ...
    json_object_object_add(__config, __key, json_object_new_int64(__value)); \
} while(0)
...
#define CONFIG_INTEGER_MUST_BE_POSITIVE(__config, __key, __fullname) do { \
    int _tmp = json_object_get_int64(json_object_object_get(__config, __key)); \
    if(_tmp < 0) { \
...
#define CONFIG_FIND_BY_NAME(__array, __name, __index_out, __item_out) do { \
    ...
    json_array_foreach(__array, _i, _item) { \
        struct json_object* _name_json = json_object_object_get(_item, "name"); \
...
#define CONFIG_ARRAY_MUST_HAVE_ITEM_NAMED(__config, __name, __array_name, __out) do { \
    ...
    json_array_foreach(__config, _i, __out) { \
        struct json_object* _name_json = json_object_object_get(__out, "name"); \
        if(!_name_json) continue; \
        const char* _name = json_object_get_string(_name_json); \
        if(_name && strcmp(_name, __name) == 0) { \
            _found = 1; \
...
#define CONFIG_IS_IN_ENUM_STRING(__config, __field_name, ...) do { \
    ...
    while(_vals[_i] && strcmp(_vals[_i], json_object_get_string(__config))) _i++; \
    if(!_vals[_i]) { \
        MARGO_ERROR(0, "Invalid enum value for \"%s\" (\"%s\")", __field_name, json_object_get_string(__config)); \
        return -1; \
    } \
} while(0)
...
#define CONFIG_NAMES_MUST_BE_UNIQUE(__array, __container_name) do { \
    ...
    for(unsigned _i = 0; _i < _len; _i++) { \
        for(unsigned _j = 0; _j < _i; _j++) { \
            struct json_object* _a = json_object_array_get_idx(__array, _i); \
            struct json_object* _b = json_object_array_get_idx(__array, _j); \
            struct json_object* _a_name = json_object_object_get(_a, "name"); \
            struct json_object* _b_name = json_object_object_get(_b, "name"); \
            if(json_object_equal(_a_name, _b_name)) { \
                MARGO_ERROR(0, "Found two elements with the same name (\"%s\") in \"%s\"", \
                        json_object_get_string(_a_name), __container_name); \
                return -1; \
            } \
        } \
    } \
...
#define CONFIG_NAME_IS_VALID(__obj) do { \
    struct json_object* _name_json = json_object_object_get(__obj, "name"); \
    const char* _name = json_object_get_string(_name_json); \
...
#define CONFIG_ADD_NEW_POOL(__pools, __name, __kind, __access) do { \
    ...
    json_object_object_add(_p, "access", json_object_new_string(__access)); \
...
#define CONFIG_ADD_NEW_XSTREAM(__xstreams, __name, __sched_predef, ...) do { \
    ...
    json_object_object_add(_s, "type", json_object_new_string(__sched_predef)); \
    struct json_object* _s_pools = json_object_new_array(); \
    unsigned _i = 0; \
    while(_pool_index[_i] != -1) { \
        json_object_array_add(_s_pools, json_object_new_int64(_pool_index[_i])); \
        _i++; \
    } \
