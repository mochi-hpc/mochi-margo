...
static inline void margo_abt_pool_destroy(margo_abt_pool_t* p)
{
    ...
    free(p->access);
    free((char*)p->name);
    if (p->margo_free_flag && p->pool != ABT_POOL_NULL) {
                         ...
                         margo_abt_t*                 a)
{
    ...
    if (primary_pool_idx == -1) { /* no __primary__ ES defined */
        if (first_abt_init) {
            ...
            if (!margo_abt_pool_init_external("__primary__", primary_pool,
                                              a->pools + a->num_pools)) {
                result = false;
                goto error;
            }
    ...
    if (primary_pool != ABT_POOL_NULL
         ...
         * as external */
        if ((!margo_abt_pool_init_external("__progress__",
    ...
    if (primary_pool != ABT_POOL_NULL && user_args->rpc_pool == primary_pool) {
        ...
        if ((!margo_abt_pool_init_external("__rpc__", user_args->rpc_pool,
                                           a->pools + a->num_pools))) {
            result = false;
            goto error;
        }
