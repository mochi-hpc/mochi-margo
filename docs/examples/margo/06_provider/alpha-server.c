...
int alpha_provider_register(margo_instance_id mid,
                            ...
                            alpha_provider_t* provider)
{
    ...
    if (flag == HG_FALSE) {
        margo_error(
            mid, "alpha_provider_register(): margo instance is not a server");
    ...
    if (flag == HG_TRUE) {
        margo_error(mid,
                    "alpha_provider_register(): a provider with the same "
    ...
    if (p == NULL) {
        margo_error(mid,
                    "alpha_provider_register(): failed to allocate memory for "
                    "provider");
        return ALPHA_FAILURE;
    }
    ...
    id = MARGO_REGISTER_PROVIDER(mid, "alpha_sum", sum_in_t, sum_out_t,
...
int alpha_provider_destroy(alpha_provider_t provider)
{
...
static void alpha_sum_ult(hg_handle_t h)
{
    ...
    alpha_provider_t      provider
        = (alpha_provider_t)margo_registered_data(mid, info->id);
