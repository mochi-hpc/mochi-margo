...
int alpha_client_finalize(alpha_client_t client)
{
    if (client->num_prov_hdl != 0) {
        margo_warning(client->mid,
                      "%d provider handles not released when "
                      "alpha_client_finalize was called",
                      client->num_prov_hdl);
    }
    free(client);
    return ALPHA_SUCCESS;
...
int alpha_provider_handle_create(alpha_client_t           client,
                                 hg_addr_t                addr,
                                 uint16_t                 provider_id,
...
{
    if (handle == ALPHA_PROVIDER_HANDLE_NULL) return ALPHA_FAILURE;
    handle->refcount += 1;
    return ALPHA_SUCCESS;
}
...
int alpha_compute_sum(alpha_provider_handle_t handle,
                      int32_t                 x,
                      int32_t                 y,
                      int32_t*                result)
{
    ...
    ret = margo_create(handle->client->mid, handle->addr,
                       handle->client->sum_id, &h);
    if (ret != HG_SUCCESS) return ALPHA_FAILURE;
